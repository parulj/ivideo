...
(function ($) {
  ...
  $.ThreeSixty = function (el, options) {
    ...
    base.loadImages = function() {
      ...
      imageName = AppCongif.domain + AppCongif.imagePath + AppCongif.filePrefix + (AppCongif.loadedImages + 1) + AppCongif.ext + ('');
    ...
    base.initEvents = function () {
      base.$el.bind('mousedown touchstart touchmove touchend mousemove click', function (event) {
        ...
        if (event.type === 'click') {
          base.$el.css("cursor", "url(img/hand_closed.png), auto");
        }
      });
      ...
      $(document).bind('mousemove', function (event) {
        if (AppCongif.dragging) {
          ...
          if(true) {
            base.$el.css("cursor", "url(img/hand_open.png), auto");
          }
        }
        base.trackPointer(event);
